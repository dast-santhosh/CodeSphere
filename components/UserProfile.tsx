...
const UserProfile: React.FC<UserProfileProps> = ({ user, lessons, onLogout, onUpdateUser }) => {
  ...
  // Find completed lessons details
  ...
  const stats = [
    ...
    { label: 'Completion Rate', value: `${progressPercentage}%`, icon: Award, color: 'text-yellow-400', bg: 'bg-yellow-400/10' },
    { label: 'Coding Streak', value: '1 Day', icon: Zap, color: 'text-orange-400', bg: 'bg-orange-400/10' },
    { label: 'Time Spent', value: `${completedCount * 1.5}h`, icon: Clock, color: 'text-purple-400', bg: 'bg-purple-400/10' },
  ];

  return (
      <div className="p-6 lg:p-10 max-w-6xl mx-auto h-full overflow-y-auto">
